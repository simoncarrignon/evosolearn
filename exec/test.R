...
E=c(x=1,y=1,z=1)
sigma=c(s=1,y=1,z=1)
test=evosolearn(100,500,omega = 2,delta = 4 ,b=2,K=200,mu=0.001,E=E,sigma=sigma)
...
omegas=seq(0,3,.5)
allos_best=sapply(omegas,function(o)replicate(100,mean(evosolearn(100,300,omega = o,delta = 2 ,b=2,K=200,mu=0.001,E=E,sigma=sigma,log=T)$pop$z)))
png("omegas_vs_z.png")
...
allos_rand=sapply(omegas,function(o)replicate(100,mean(evosolearn(100,300,omega = o,delta = 2 ,b=2,K=200,mu=0.001,E=E,sigma=sigma,type="random",log=T)$pop$z)))
...
osnds_s=parSapply(cl,omegas,function(o)sapply(deltas,function(d){print(paste(o,d));mean(replicate(50,mean(evosolearn(100,10,omega = o,delta = d ,b=2,K=100,mu=0.001,E=E,sigma=sigma,log=F)$pop$z,na.rm=T)),na.rm=T)}))
osnds_rand=parSapply(cl,omegas,function(o)sapply(deltas,function(d){print(paste(o,d));mean(replicate(50,mean(evosolearn(200,1000,type="random",omega = o,delta = d ,b=2,K=200,mu=0.001,E=E,sigma=sigma,log=F)$pop$z,na.rm=T)),na.rm=T)}))
stopCluster(cl)
...
bigosnds_popsize=parApply(cl,combined,1,function(i){o=i[1];d=i[2];print(paste(o,d));mean(replicate(100,nrow(evosolearn(1000,2000,omega = o,delta = d ,b=2,K=1000,mu=0.0001,E=E,sigma=sigma,log=F,type="best")$pop)),na.rm=T)})
bigosnds_rand_popsize=parApply(cl,combined,1,function(i){o=i[1];d=i[2];print(paste(o,d));mean(replicate(100,nrow(evosolearn(1000,2000,omega = o,delta = d ,b=2,K=1000,mu=0.0001,E=E,sigma=sigma,log=F,type="random")$pop)),na.rm=T)})
...
t=evosolearn(n,tstep,omega = 0,delta = 0 ,b=2,K=200,mu=c(x=0.01,y=0,z=0),E=c(x=.01,y=0,z=0),sigma=c(s=1,y=1,z=1),log=T,type="best",pop=pop)
...
tstep=200
pop=generatePop(n,distrib=list(x=runif(n,-1,1),y=rep(0,n),z=rep(0,n)))
t=evosolearn(n,tstep,omega = 0,delta = 0 ,b=2,K=200,mu=c(x=0.01,y=0,z=0),E=c(x=.01,y=0,z=0),sigma=c(s=1,y=1,z=1),log=T,type="best",pop=pop,allpops=T)
