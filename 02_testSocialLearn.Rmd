...
plot(1,1,log="xy",xlab="phenotype frequencies",ylab="ccfd (%)",xlim=c(1,10000),ylim=c(0.01,100),type="n",main=bquote(mu == .(mu) ~ N == .(n)))
for(i in 1:10){
    soci=simpleEvoModel(pop = pop, tstep = 50,log=T,theta=rep(0,1000),sigma=c(s=10000,y=10000,z=10000),omega=0,delta=0,b=2,K=n,m=c(x=.1,y=0,z=0),mu=c(x=mu,y=0,z=0),allpops=T,repro="asex",selection=T,statvar=c("p","x","z"),statfun="mean",E=c(x=0,y=0,z=0),sls="random") 
    u=unlist(sapply(soci$allpop,function(s)s[,"p"]))
    freqt=table(u)
    cc=ccfd(freqt)
    lines(cc[,"x"],cc[,"y" ],col="green",lwd=2)
...
* $\sigma_y =0.8$
* $\sigma_s =4$
* Genotype of Individual Leaners $i$:  $y=1,z=0$
* Genotype of Social Leaners $s$:  $y=0,z=1$

Thus :

* fitness of individual learner $i$ when match is: $w_i=$ `r round(fitness(1,1,0,1,0,.2,.8,4),digits=2)`
* fitness of social learner $s$ when match is: $w_s=$ `r round(fitness(1,1,0,0,1,.2,.8,4),digits=2)`
* fitness of $i$ when mismmatch is: $w_i=$ `r round(fitness(0,1,0,1,0,.2,.8,4),digits=2)`
* fitness of $s$ when mismatch is: $w_s=$ `r round(fitness(0,1,0,0,1,.2,.8,4),digits=2)`
...
dev.off()
```
```{r ,rogers1988,out.width="50%",fig.align="center",fig.cap="Rogers (1988), Fig 1. "}
knitr::include_graphics("images/rogerFig1.png")
```

```{r rogerRepro,out.width="50%",fig.align="center",fig.cap="Illustration of the Rogers effect"}
