...
png(paste0("ccfdPhenotype_MU",mu,"_N",n,".png")) 
plot(1,1,log="xy",xlab="phenotype frequencies",ylab="ccfd (%)",xlim=c(1,10000),ylim=c(0.01,100),type="n")
for(i in 1:10){
...
allimages=lapply(allimage,unlist)
```
...
plot(1,1,log="xy",xlab="phenotype frequencies",ylab="ccfd (%)",xlim=c(1,100000),ylim=c(0.1,100),type="n")
for(i in 1:10){
...
dev.off()
```
```{r ,rogers1988,fig.cap="Rogers (1988), Fig 1. "}
...
dev.off()
```
```{r rogerBest,fig.cap="When social learning is able to spot the best individual the Rogers effect disappear"}
...
```{r rogerEvo,fig.cap="Rogers paradox consequence: social learning isn't selected when environmental change is relatively high"}
...
```{r rogerEvoslow,fig.cap="When environment change is slow,social learning is selected"}
