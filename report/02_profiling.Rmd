...
tstep=100
theta=environment(tstep,.1,0)

for(strat in strategies){
    df <- "test.log"
    Rprof(df, memory.profiling = TRUE)
    evodf=evosolearn(b=2,K=1000,m=,mu=mu,E=E,sigma=sigma,log=F,sls=strat,theta=theta,pop=pop)
    plot(evodf[,"mean_x"],type="l",ylab=pureGl,main=paste("strategie:",strat))
    plot(evodf[,"mean_y"],type="l",col=pureIl,ylim=c(0,1))
    lines(evodf[,"mean_z"],type="l",col=pureSl)
    legend("topleft",legend=c("mean_y","mean_z"),lty=1,col=c(pureIl,pureSl))
    Rprof(NULL) ; 
    sdf=summaryRprof(df)
    rmarkdown::paged_table(as.data.frame(sdf$by.total))
}
