...
```{r fitprop-limited,cache=TRUE,fig.show="hold",fig.cap="Compare effect of choosing among a subgroup of the population. 1000 Agents"}
n=500
pop=generatePop(n)
pop[,"p"]=sample.int(200,n,replace=T)
pop[,"x"]=0
...
```{r profiling-fitprop,fig.cap="profilingfitprop", cache=TRUE}
timings=lapply(maxns,function(mn){
         microbenchmark(
            evosolearn(pop=pop,tstep=tsteps,theta=rep(theta,tsteps),b=2,K=100,m=c(x=0,y=0,z=0),mu=c(x=0,y=0,z=0),E=c(x=0,y=0,z=5),sigma=c(s=100,y=100,z=100),log=F,sls="fitprop",allpop=F,selection=F,repro="unique",maxn=mn),
                            times=5,
                            unit="s")
}
             )
