...
As we have seen, every dataset gives us proxy for the climate at different resolutions, whcih means that using the environment as is in our simulation will creat generation spanning uneven time. We need to interpolate new datapoint to fill the gaps. 

The biggest gap are in the LR04 stack and epica,, the ones describing the largest time scale and are respectively of `r max(getDateResolution(lr04$year))` years. But the length of those gap varying infrom each dataset, here is the distribution
...
We can see that most of the gaps last betwen 20 to 50 years, and some of them last mor than 200 years.

Mitsui et al 2019 have shown that for cycles betweeen $10^{2.5}$  to $10^{3.5}$ the amplitude of cycles of decreasing length is approximate by a log-log linear fit of slope -1.41 wherease the ampltitude of shorter cycle ($<100$) is closer to white noise. 

To know the amplitude (ie the variation) of the noise to add, we compute the standard variation of sliding windows of different sizes -- orresponding to what would be  gap of different size, using the data from NGRIP2.
...
To generate new datapoints we use our function to generate $1/f$ noise with slope $\omega$ and standard deviation $\delta$ when the gaps are bigger than 100 years, for lower gaps we add white noise. Both type of noise are then added to the straight line between the points around the gap.


For all environment, we aim at having a final resolution of 20 years. All environments need to be rescaled to same scale variations; this has been done by using the Vostok estimates of temperature (cf section \@ref(general) and \@ref(rescale))

Quick summary of the process:

1. We go from all data measurment and the date associated
2. When two measures are separated by less than 100 years( 5 times the resolution we want), but by a distance more than twice the highest resolution (40years), we add datapoints (3 if the resolution wanted is 20 years). Those data point are added to be regularly spaced on a straight line between the two orignal points. Then noise is added to the point, the noise being  calculated as $N(0,.38)$ , 0.38 (mdw=`r mdw`) being the mode of the standard variation calculated in windows of 100years from ngrip2 shown in the Figure\@ref(fig:deltaW).
3. When two measures are separated by less than 20 years we keep the original dataset
4. When two measures are separated by more than 100 years we add regularly spaced datapoints that follow the a straight line between the two orignal points. Then we add noise, but this time generate using the function `environment` which generate noise given a slope $\omega$ and a standard variation $\delta$. We use the slope calculaded by Mitsui et al. of $1.41$ as the cycles generated willl be all between 100 to 6000 years. For the detla we use this time the value calculated for 1000 years windows which gives a value of $\delta=1.35$
   Here is the central central equationn used to generate $1/f$-noise, from a time serie of normally distributed noise  ($N(0,1)$) $Y(t=1,...,T)$ to a $1/f$ noise of slope $\omega$ and standard deviation $\delta$.
...
The equation  is implemented in the function `environment` and ($S^{-1}[S(Y)^{-\omega}]$) is implmented by `TK95`, borrowed from the package from tuneR package,  based on [Timmer and Koening 1995](https://ui.adsabs.harvard.edu/abs/1995A\%26A...300..707T/abstract)'s paper.
...
u=lr04[100:101,]
inter=interpolate(u$dTsVscales,u$year,50) 
intLin=inter$data
...
Once applied to all dataset:

```{r interpolNgripFull, fig.width=14,out.width = "100%",fig.cap="interpolation linear + noise full dataset LR04", fig.link="_main_files/figure-html/interpolNgripFull-1.png"}

    newt=interpolate(lr04$dTsVscales,lr04$year,finalres=250)$data
	#linear interpolation
    newy=seq(min(lr04$year),max(lr04$year),250)
	#linear + noise
    newtN=interpolate(lr04$dTsVscales,lr04$year,finalres=250,omega=omw,delta=1.31,delta2=.38)$data
...
Let see know how the parameter of our  interpolation impact the properties of our environments. $X_{ni}$ represents the metrics for the environment with the noisy missing point, $X_{li}$ for simple linear interpolation.
