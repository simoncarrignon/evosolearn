...
To know the amplitude (ie the variation) of the noise we need to add to the line, we compute  the variation of sliding windows of the size of the gap.
...
To generate new datapoints we use our function to generate noise of a certain slope when the gap are bigger than 100 years and for gap smaller than 100 years we generate a white noise. Both type of noise are then added to the straight line between the points around the gap.


For all environment, we aim at having a final resolution of 20 years. All environment should be rescaled at in to have variation at comparable scale ; this hav ebeen done by rescaling all environment using the Vostok estimates of temperature.
...
From this we can interpolate new points. To do so we take the linear interpolation and  add  a noise generated given the value of $\omega$ and $\delta$ as calculated before. Let's check an exemple using the data point 100 and 101  from the lr04 stack, which represent the temperature scaled to Vostok temperature from $\delta^{18}O$ for the year `r lr04$year[100]` and the year `r lr04$year[101]`
...
```{r interpolNgripFull, fig.width=14,out.width = "100%",fig.cap="interpolation linear + noise full dataset", fig.link="_main_files/figure-html/interpolNgripFull-1.png"}
	...
	#linear + noise
    newtN=interpolate(lr04$dTsVscales,lr04$year,finalres=250,omega=omw,delta=mdw)$data
...
In the figure below we apply this method to generate missing data for a few dataset. The red curve represent the dataset subsampled at is lowest resolution. The black line represent the dataset plus the 
new data points while the green line represent the spectrum for the ngrip2 dataset.
