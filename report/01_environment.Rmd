...
```{r deltacheck,fig.cap="Environment generator changing $\\delta$",cache=TRUE}
t=1000 #number of timestep
...
* Martrat: Iberian Margin sediments , $ U^{k'}_{37} $ alkenone- SST (Sea Surface Temperature), [Martrat et al (2007)](https://science.sciencemag.org/content/317/5837/502)

```{r allenvironment,out.width="90%",fig.width=10,fig.height=8,fig.show="hold",fig.cap="preview of all datasets scaled with martrat total length (note that lr04 goes almost 10 times before martrat and is thus hidden here)",fig.link="_main_files/figure-html/allenvironment-1.png",cache=TRUE}
...
To translate measured $\delta^{18}O$ in temperature Petit et al. use the fact :

> the isotopic content of snow in East Antarctica ($\delta D$ or $\delta^{18}O$) is linearly related to the temperature above the inversion level, $T_I$, where precipitation forms, and also to the surface temperature of the precipitation site, $T_S$.

While the inversion level and the surface temperature are likned by the relation $\Delta T_I=0.67\Delta T_S$, while the relation between the isotop fraction and the ice formed in the atmosphere and the inversion level is given by:

$$ \Oelta T_I = (\Delta \delta D_{ice} − 8\Delta\delta^{18}O_{sw})/9 $$

where $\Delta\delta^{18}O_{sw}$ is the globally averaged change from today's value of seawater $\delta^18_O$

The, authors use sediment records from Bassinot et al. 1994 to estimate the past value for $\delta^{18}O_{sw}$.
```{r real-autocorelation,fig.show="hold",out.width="33%",fig.cap="Visual representation of the autocorleation of each environment.",cache=TRUE}
...
### Scale of variation {#rescale}

As we can see, the scales of change of every environment isn't the same, as they rely on the measure of different isotopoes, which in turne are correlated throught different relation to temperature, as we have seen beween the ice $\delta_D$ and benthic $\delta^{18}O$ which both use different equations.

Nontheless, minimal and maximal variations are reached roughly at the same time suggesting  those variation could  be rescaled one scale to another. 
to rescale all in siilar environment we match the variation of the changes during the time for VOSTOK dataset at the same scale as VOSTOK.


```{r rescale,out.width="80%",fig.show="hold",fig.width=8,fig.height=6,fig.cap="Rescale following Vostok Temperature, exept for Martrat dataset which has it's own temperature scale that we will use for following experiments",cache=TRUE}
...
As we can see this has no impact on the autocorrelation between time, it rescaled in a good way all environments, resulting  in $\delta$ between 2 and 3. On the other hand, impact of the slope of the noise isn't straightforward.
...
```{r resample,out.width="80%",fig.show="hold",fig.width=8,fig.height=6,fig.cap="Uniform Sampling: black curve represents the original dataset, green curve the result of the function getClosest, red curve the results of the getMean2 function",cache=TRUE}
...
```{r reScale,out.width="50%",fig.show="hold",fig.width=8,fig.height=6,fig.cap="Uniform Sampling as before, zoom on the begining(left) and end (right) of the time series",cache=TRUE}
par(mfrow=c(length(namesRealEnv),1))
for(e in namesRealEnv){
    env=allRealEnv[[e]]
    par(mar=c(3,4,0,2))
...
mtext("year",1,2)
```
We report in the following table the properties of the environment once applied the resampling. $X_v$ represent the rescaled dataset, $X_{gc}$ the rescaled + resampled following `getClosest` and $X_{gm}$ rescaled + resampled following `getMeans2` function.
...
```{r, out.width = "45%",fig.cap="distributions of $\\delta$ for 500 years time series in data from NGRIP dataset",fig.show="hold",eval=F}
ngrip=read.csv("data/ngrip.csv")
...
```{r fullspec-fig2,out.width="50%",fig.show="hold",fig.cap="Mitsui et al but with the full NGRIP2 dataset",fig.show="hold"}

m.spec=getSpectrumMTM(ngrip2$dOice,ngrip2$year)
plot(log10(m.spec$freq),log10(m.spec$spec),xaxs="i",type="l",xlab=expression(log[10]*f),ylab=expression(log[10]*P(f)))
...
Comparison of all rescaled and subsampled spectrum: 

```{r allScales,out.width="80%",fig.cap="Power Spectral Density (usin MTM) for all rescaled envrionments"}
...
## Combine Real Data with simulated noise

As we have seen, every dataset gives us a proxy for temperature changes at different resolutions and scales. 

Thus, all  environments need to be rescaled to same scale variations; this has been done by using the Vostok estimates of temperature (cf section \@ref(general) and \@ref(rescale))

Then, as for each environment the time interval at which measures are available is chaning, thus we can't use neither directly the measures for  our simulation. When records are uneven, we need to interpolate new datapoints to fill the gaps. 

The biggest gaps are in the LR04 stack and epica, the datasets describing the largest time scales, and are respectively of `r max(getDateResolution(lr04$year))` years. But the length of those gap varies between each dataset. Here is the distribution of the length of time interval for all datasets:

```{r gapDistrib,fig.cap="Distribution of gap lenght in year for all environments"}
plot(density(unlist(sapply(allRealEnv,function(e)getDateResolution(e$year)))),xlab="gap size (years)",ylab="density",log="x",main="distribution of gap sizes")
```

We can see that most of the gaps last betwen 20 to 50 years, but some of them are bigger than 200 years.

One good thing is that Mitsui et al 2019 have shown that for cycles betweeen $10^{2.5}$  to $10^{3.5}$ years, the amplitude of cycles of decreasing length is approximated by a log-log linear fit of slope -1.41; while the ampltitude of shorter cycle ($<100$) is closer to white noise. 

To know the amplitude (ie the variation) of the data we need to add to fill the gap, we compute the standard variation of sliding windows of different sizes in years -- corresponding to what would be gaps of different size. We do this using NGRIP2 as it doesn't contain any gap bigger than 20 years.

```{r deltaW,out.width="33%",fig.show="hold",fig.cap="variation of the standard deviation for different windows size. The red line represent the mode of the distribution."}
for(w in c(100,500,1000)){
    dw=getWindows(ngrip2$dTsVscales,w,sd)
    #mdw=hdr(dw)[["mode"]]
    #saveRDS(file=paste0("data/dt",w,".bin"),mdw)
    plot(density(dw),ylab="proba",xlab=expression(delta),main=paste("w=",w))
    mdw=readRDS(file=paste0("data/dt",w,".bin"))
    abline(v=mdw,col="red")
    text(mdw,0,round(mdw,digits=2),col="red",pos=4)
}
mdw=readRDS(file=paste0("data/dt",100,".bin"))
```

To generate new datapoints we use the function that generates $1/f$-noise with slope $\omega$ and standard deviation $\delta$. When the gaps are bigger than 100 years we use an $\omega=1.41$ and $\delta=1.37$, which is the mode of the standard deviations of variation measured for windows of 500 to 1000 years. For lower gaps we add white noise ($\omega=0$) with $\delta=.38$, the mode of the standard deviation of variation in moving windows of 100 years.  Both type of noises are added to the straight line between the points around the gap.

Here is how those noises are added step by step:

1. We go from all data measurment and the date associated
2. When two measures are separated by less than 100 years(5 times the resolution we want), but by a time more than twice the highest resolution (40years), we add datapoints (3 if the resolution wanted is 20 years). Those data point are regularly spaced on a straight line between the two orignal points. Then noise is added to the points. This noise is calculated as $\mathcal{N}(0,.38)$ , 0.38 (mdw=`r mdw`) being the mode of the standard variation calculated in windows of 100years from ngrip2, as shown in the Figure\@ref(fig:deltaW).
3. When two measures are separated by less than 20 years we keep the original data points.
4. When two measures are separated by more than 100 years, we add regularly spaced datapoints following the a straight line between the two orignal points. Then we add noise, but this time, the noise is generated using the function `environment`, which generates $1/f$-noise given a slope $\omega$ and a standard variation $\delta$. We use the slope calculaded by Mitsui et al. of $1.41$ as the cycles generated at this current step will be all between 100 to 6000 years. For $\delta$ we use the value calculated for 1000 an 500 years windows, which gives a value of $\delta\approx1.37$
   Here is the central equation used to generate the $1/f$-noise of slope $\omega$ and standard deviation $\delta$ from a time serie of normally distributed noise ($\mathcal(0,1)$) $Y(t=1,...,T)$:    
   $$ y=\frac{ \delta \times S^{-1}[S(Y)^{-\omega}]}{SD(S^{-1}[S(Y)^{-\omega}])} $$  (from Whitehead 2007)\\
 $S$ is the Fourier transform, and $S^{-1}$ the inverse Fourier transform .  
The equation  is implemented in the function `environment` and ($S^{-1}[S(Y)^{-\omega}]$) is implemented by `TK95`, borrowed from the package from tuneR package,  based on [Timmer and Koening (1995)](https://ui.adsabs.harvard.edu/abs/1995A\%26A...300..707T/abstract)'s paper.


Using this process we can add data points when the datasets are too sparse. Nonetheless, as some datasets have very unequal sampling and higer resolution than 20 years (16,17 year in Vostok), it's still usefull to resampled the data using the functions presented above. In the current implementation, we do so using the function `getClosest`s, that  generates an evenly spaced vector of dates and we associated to each one the closest in the real dataset.  We could use the mean of the datapoints falling withing the new interval but it's very rare that we have more than one points (3 cases within all datasets).

Let's check an exemple using the data point 100 and 101 from the lr04 stack, which represent the temperature scaled to Vostok temperature from $\delta^{18}O$ for the year `r lr04$year[100]` and the year `r lr04$year[101]`

```{r, fig.width=7,out.width = "45%",fig.cap="step for interpolating gaps",fig.show="hold",cache=TRUE}
u=lr04[100:101,]
inter=interpolate(u$dTsVscales,u$year,20) 
intLin=inter$data
reald=c(1,length(intLin)) #real data to be omited
...
omw=1.41
nois=environment(length(intLin),omega=1.41,delta=1.37)
plot(datin[-reald],nois[-reald],ylab="unit of noise",xlab="year",pch=20,col=2,type="b")
plot(u$year,u$dTsVscales,ylab=expression(delta^18*O),xlab="year",pch=20,ylim=range(intLin+nois))
points(datin[-reald],intLin[-reald],col="green",pch=20,type="b")
points(datin,c(u$dTsVscales[1],intLin[-reald]+nois[-reald],u$dTsVscales[2]),col="red",pch=20,type="b")
points(u$year,u$dTsVscales,pch=20)
```

Once applied to full dataset:

```{r interpollr04, fig.width=14,out.width = "100%",fig.cap="interpolation linear + noise full dataset LR04", fig.link="_main_files/figure-html/interpolNgripFull-1.png",cache=TRUE}

#linear
    newt=interpolate(lr04$dTsVscales,lr04$year,finalres=20)
#noisy
    newtN=interpolate(lr04$dTsVscales,lr04$year,finalres=20,omega=omw,delta=1.31,delta2=.38)
    plot(lr04$year,lr04$dTsVscales,type="l",lwd=5,xlab="years",ylab="scaled delta temp",ylim=range(newtN$data))
    lines(newtN$year,newtN$data,type="l",col="red",lwd=2)
    lines(newt$year,newt$data,type="l",col="green")
```
...
```{r zoomInterpolLR04, fig.width=4,fig.height=4,out.width = "40%",fig.cap="Zoom on the first (left) and last (right) 30,000 years interpolation linear + noise",fig.show="hold"}
for(xl in list(c(-5320000,-5290000),c(-30000,0))){
    plot(lr04$year,lr04$dTsVscales,type="l",lwd=5,xlab="years",xlim=xl,ylim=range(newtN$data),ylab="scaled delta temp")
    lines(newtN$year,newtN$data,type="l",col="red",lwd=2)
    lines(newt$year,newt$data,type="l",col="green")
}
```

Now for all environments:
```{r interpolallEnv, fig.width=14,out.width = "100%",fig.cap="interpolation linear + noise full dataset",cache=TRUE,fig.show="hold"}

for(e in names(allRealEnv)){
    env=allRealEnv[[e]]
    newt=interpolate(env$dTsVscales,env$year,finalres=20)
    newtN=interpolate(env$dTsVscales,env$year,finalres=20,omega=omw,delta=1.31,delta2=.38)
    plot(env$year,env$dTsVscales,type="l",lwd=5,xlab="years",ylab="scaled delta temp",ylim=range(newtN$data),main=e)
    lines(newtN$year,newtN$data,type="l",col="red",lwd=2)
    lines(newt$year,newt$data,type="l",col="green")
}
```
...
Let see know how the parameters of our interpolation impact the properties of our environments. $X_{ni}$ represents the metrics for the environment with the noisy missing point, $X_{li}$ for simple linear interpolation.
...
summaryEnv=sapply(allRealEnv[-3],function(env)
                  {
                      ...
                      t=2:(length(env[,2]))
                      correl=cor(env[t,2],env[t-1,2])
                      ni=interpolate(env$dTsVscales,env$year,20,omega=1.788783,delta=0.09894122)$data
                      li=interpolate(env$dTsVscales,env$year,20)$data
                      t=2:(length(ni))
...
summaryEnv=sapply(allRealEnv[-3],function(env){
       ...
       list(
            "res"=resolution,
            "res_{li}"=paste(unique(getDateResolution(li$year))[1],"year"),
            "res_{ni}"=paste(unique(getDateResolution(ni$year))[1],"year"),
            "#records"=length(env[,2]),
            "#rec_{li}"=length(li$data),
            "#rec_{ni}"=length(ni$data),
            "oldest record"=min(env$year)
            )
}
)
...
In the figure below we apply this method to generate missing data for a few dataset. The red curve represent the dataset subsampled at is lowest resolution. The black line represent the dataset plus the new data points, the green line represent the spectrum for the ngrip2 dataset.


```{r resample-vostok,fig.cap="Interpoling missing data for different environment using NGRIP2 Power Sprectral Density",out.width="50%",fig.show="hold",cache=TRUE}
#tstv=interpolate(theta=vostok$dTsVscales,times=vostok$year,finalres=20,delta=1.2,omega=1.41,delta2=0.38)
#tstv=getClosest(tstv$data,tstv$year,by=20)
#saveRDS(file="data/vostokInterpol.rds",tstv)
#tstv=interpolate(theta=martrat$dTsVscales,times=martrat$year,finalres=20,delta=1.2,omega=1.41,delta2=0.38)
#tstv=getClosest(tstv$data,tstv$year,by=20)
#saveRDS(file="data/martratInterpol.rds",tstv)
#tstv=interpolate(theta=epica$dTsVscales,times=epica$year,finalres=20,delta=1.2,omega=1.41,delta2=0.38)
#tstv=getClosest(tstv$data,tstv$year,by=20)
#saveRDS(file="data/epicaInterpol.rds",tstv)
#tstv=interpolate(theta=lr04$dTsVscales,times=lr04$year,finalres=20,delta=1.2,omega=1.41,delta2=0.38)
#tstv=getClosest(tstv$data,tstv$year,by=20)
#saveRDS(file="data/lr04Interpol.rds",tstv)
for(env in c("vostok","martrat","epica","lr04")){
    tstv=readRDS(file=paste0("data/",env,"Interpol.rds"))
    i=get(env)
    rs=getClosest(i$dTsVscales,i$year,max(getDateResolution(i$year)))
    plotSpectrum(tstv$data,tstv$year,xlim=c(-6.5,-1.5),main=env) 
    plotSpectrum(rs$data,rs$year,col="red",add=T)
    plotSpectrum(ngrip2$dTsVscales,ngrip2$year,add=T,col="green")
legend("topright",legend=c(paste(env,"+ interpolate"),env,"ngrip2"),lwd=1,col=c("black","red","green"))
}
```

<!--It should be noted that most of the gaps will be of length between 20 to 500 years, which means they will include only cycles of high frequencies. If we look at the spectrogram of NGRIP (Fig \@ref(fullspec-fig2) the cycles at those frequencies are the one with very low "power", thus  corresponding to very small changes in degree, the biggest changes are brought by large, very low frequency cycles.-->

Comparison of different noise interpolation:

```{r}
listimage=c(
"images/verticalMz_vostokinterpolate_LinFALSERes20SlsrandomLOW_sigma2_E0.6.png",
"images/verticalMz_vostokinterpolate_LinFALSERes20SlsrandomBIS2_sigma2_E0.6.png",
"images/verticalMz_vostokinterpolate_LinTRUERes20SlsrandomBIS_sigma2_E0.6.png",
"images/verticalMz_vostokinterpolate_LinTRUERes20SlsrandomTRIS_sigma2_E0.6.png",
"images/verticalMz_vostokinterpolate_LinTRUERes20SlsrandomQUIS_sigma2_E0.6.png"
)
```
...
<!-- Some interpolation old garbage:
...
```{r, out.width = "45%",fig.cap="distributions of $\\omega$ for 500 years time series in data from Andersen 2004/NGRIP project",fig.show="hold",eval=FALSE}
...
As the scale between the two environments are now scaled to match Vostok Temperature , the variations (_ie_ $\delta$)  can be compared. Thus I will use the mode of the distribution of $\delta$ for 500 years windows from NGRIP dataset.
