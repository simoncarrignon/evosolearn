...
```{r mtisui-fig2,fig.out="50%",fig.cap="redo Mitsui et al 2019 analysis"}
...
fitw=lm(y[w] ~ x[w])
abline(fitw)
text(-2.5,3,bquote(omega == .(round(fitw$coefficients[[2]],digit=2))))


middle=seq(-4.5,max(x),.1)
allOmegas=sapply(middle,getOmega2,x=x,y=y)
plot(middle,allOmegas,type="b",xaxs="i",xlab=expression(log[10]*f),ylab=expression(omega))

knitr::include_graphic("images/dzy01102.jpeg")

```

```{r allScales,fig.out="50%",fig.cap="redo Mitsui et al 2019 analysis"}
