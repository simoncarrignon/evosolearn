...
### the following can be used to regenerate environment from their original datasets
    ...
    ngrip2$dOice=ngrip2R$d180.NGRIP2 #delta 18 O ice from greenland
    ngrip2=as.data.frame(ngrip2)
    ngrip2=ngrip2[order(ngrip2$year),]
    ngrip2=ngrip2[c(1,seq(2,nrow(ngrip2),2)),]
...
######
   ...
   dataEnv=list("epica"=expression(delta*D),"lr04"=expression(delta[18]*O),"ls16"=expression(delta[18]*O),"vostok"=expression(delta*D),"ngrip"=expression(delta*D),"martrat"=expression(S*S*T))
    ...
    for(i in names(allRealEnv)){
        ...
        if(min(env$year)>oldest){
            axis(s,pos=min(env$year)) #yaxis 
            mtext(dataEnv[[i]],s,pos=min(env$year),cex=.8) #yaxis title
        }
        else{
            axis(s,1)
            mtext(dataEnv[[i]],s,2,cex=.8) #yaxis title
        }
        #side=side+1
    }
    axis(1)
